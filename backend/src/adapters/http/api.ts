...
export function registerApiRoutes(
  ...
  app.get('/routes/comparison', async (_req: Request, res: Response) => {
    const all = await deps.routes.findAll();
  ...
  app.post('/routes/:id/baseline', async (req: Request, res: Response) => {
  ...
  app.get('/compliance/cb', async (req: Request, res: Response) => {
    ...
    const all = await deps.routes.findAll();
    const filtered =
      yearFilter != null ? all.filter((r) => r.reportingYear === yearFilter) : all;
  ...
  app.post('/banking/bank', async (req: Request, res: Response) => {
    ...
    const cbBefore = await aggregateEffectiveComplianceBalance(
      allRoutes,
      deps.banking,
      deps.policy,
      yearScope,
    );
    const outcome = await handleBanking(
      route,
      { routeId, amountMjEquivalent },
      deps.banking,
      deps.policy,
    );
    if (!outcome.ok) {
    ...
    const cbAfter = await aggregateEffectiveComplianceBalance(
      allRoutes,
      deps.banking,
      deps.policy,
      yearScope,
  ...
  app.post('/banking/apply', async (req: Request, res: Response) => {
    ...
    const route = await deps.routes.findById(routeId);
    const allRoutes = await deps.routes.findAll();
    const outcome = await handleApplyBanked(
      route,
      { routeId, appliedMjEquivalent },
      deps.banking,
      deps.policy,
      allRoutes,
      year,
  ...
  app.post('/pools', async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(memberRouteIds) || memberRouteIds.some((x: unknown) => typeof x !== 'string')) {
      res.status(400).json({ error: 'memberRouteIds must be an array of strings' });
      return;
    }
    const created = await createPoolWithValidation(
      { name, reportingYear, memberRouteIds },
      deps.routes,
      deps.policy,
    );
    if (!created.ok) {
